  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (abort) {
      ...
      if (shouldAbort !== null && shouldAbort !== undefined && shouldAbort !== false) {
        verbose && logger.warn("Batch abort", shouldAbort);
        break;
      }
    }
    ...
    verbose && logger.info(`Start task: ${item}`);
    ...
    const activeTask = new Promise<N>((resolve, reject) => {
      ...
      .then((response: N) => {
        ...
        verbose && logger.info(`End task: ${item}`);
      })
      .catch(error => {
        activeTasks.splice(activeTasks.indexOf(activeTask), 1);
        verbose && logger.info(`End task: ${item}`, error);
        throw error;
      });
